...
public interface FriendRepository extends JpaRepository<Friend, FriendPK> {
	...
	@Query(value = "select p from Profile p inner join p.friends f where f.status"
			+ " = :status and (f.id.profile.id = :profileId or f.id.friend.id = :profileId)")
	Page<Profile> findAllFriends(@Param(value = "status") FriendStatus status, @Param(value = "profileId") Long profileId,
			Pageable pageable);
